...
function getServerManifest(route, context) {
    ...
    function getFlatNodes(route, parentRoute = '') {
        ...
        if (!route.loaderContextKey && route.type === 'route') {
            ...
            if (routeModule?.loader && typeof routeModule.loader === 'function') {
                // Only resolve paths when exporting, keep original paths in dev mode
                const resolvedPath = context?.isExporting
                    ? (0, resolveLoaderPath_1.resolveLoaderModulePath)(route.contextKey, context)
                    : route.contextKey;
                route.loaderContextKey = resolvedPath;
                // Store the resolved path for reuse by generated routes
...
function getMatchableManifestForPaths(paths) {
    return paths.map(([normalizedRoutePath, absoluteRoute, node]) => {
        ...
        if (node.loaderContextKey) {
            // For generated routes, use the resolved path from the parent dynamic route
